...
[:documents, :pages].each do |hook|
  Jekyll::Hooks.register hook, :post_render do |item|
    ...
    # Matches project <blockquote> open/close tags alongside <a> tags so that
    # links nested in a `{: type="project" }` block are always opened in a new
    # tab, whatever domain they point to (e.g. the mathiasd.fr demo subdomains).
    content.gsub!(%r{<blockquote\b[^>]*>|</blockquote>|<a\b([^>]*?)\bhref=(["'])([^"'>]+)\2([^>]*)>}i) do |match|
      pre_attrs  = $1 # attributes before href
      quote      = $2
      href       = $3
      post_attrs = $4 # attributes after href

      if match =~ %r{\A<blockquote}i
        inside_project = match =~ /\btype\s*=\s*(["'])project\1/i
        match
      elsif match =~ %r{\A</blockquote>}i
        inside_project = false
        match
      else
        ...
        if (pre_attrs + post_attrs) =~ /\brel\s*=/i || (pre_attrs + post_attrs) =~ /\btarget\s*=/i
          full_tag
        # outside a project: whitelist by scheme (mailto:, tel:) or domain substring
        elsif !inside_project &&
              (whitelist_schemes.any? { |s| href.start_with?(s) } ||
          ...
          is_local = false
          if inside_project
            # every link from a project is external, whatever its domain/path
